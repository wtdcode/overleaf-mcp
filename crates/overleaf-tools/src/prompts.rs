...
pub const EDIT_FILE: &str = "Replace an exact string in an Overleaf doc with new text, applied as a live collaborative edit (other people's concurrent edits are preserved). `old_string` must match the current content exactly and, unless `replace_all` is set, must be unique in the file. Requires a prior read_file of the same file in this session; if the content changed remotely and `old_string` no longer matches, re-read the file and retry. Characters outside the Unicode BMP (e.g. emoji) cannot be stored by Overleaf and are rejected. After finishing a round of edits, run compile to verify the project still builds.";
